...
namespace VBScriptTranslator.RuntimeSupport.Implementations
{
	...
	public class DefaultArithmeticFunctionalityProvider
	{
		...
		public object DIV(object l, object r)
		{
			...
			// Coerce the left and right values into Double-precision numbers for doing the maths
			// DateTime get special treatment to match VBScript behaviour where the underlying "value" is the total number of days of the DateTime
			var lDate = TryToCoerceInto<DateTime>(l);
			var rDate = TryToCoerceInto<DateTime>(r);
			var lDouble = (lDate != null) ? lDate.Value.Subtract(VBScriptConstants.ZeroDate).TotalDays : AsDouble(l);
			var rDouble = (rDate != null) ? rDate.Value.Subtract(VBScriptConstants.ZeroDate).TotalDays : AsDouble(r);

			// Throw an error if the divisor (right-hand-side) is 0. For the case where the left-hand-side is non-zero, a Division-by-zero error is thrown, however if the
			// left-hand-side is also zero, an Overflow error is thrown instead.
			if (rDouble == 0)
			{
				if (lDouble == 0)
					throw new VBScriptOverflowException();
			...
			{
				if (dblResult >= float.MinValue && dblResult <= float.MaxValue)
					return (float)dblResult;
			}

			// All is well, return the result as a Double
			return dblResult;
		}
