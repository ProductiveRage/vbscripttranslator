...
namespace VBScriptTranslator.RuntimeSupport.Compat
{
	...
	public class TranslatedPropertyIReflectImplementation : BasicIReflectImplementation
	{
		...
		private readonly IEnumerable<PropertyInfo> _representedPrivateProperties;
		public TranslatedPropertyIReflectImplementation()
		{
			_representedPublicProperties = GetTranslatedPropertyRepresentations(this.GetType(), Accessibility.Public);
			_representedPrivateProperties = GetTranslatedPropertyRepresentations(this.GetType(), Accessibility.Private);
		}
		...
		public override PropertyInfo[] GetProperties(BindingFlags bindingAttr)
		{
			...
			if (IsBindingRequestFor(bindingAttr, BindingFlags.NonPublic | BindingFlags.Instance))
				properties = _representedPrivateProperties.Concat(properties).ToArray();
			return properties;
		}
		...
		private PropertyInfo TryToGetRepresentedProperty(string name, BindingFlags bindingAttr)
		{
			...
			if (representedProperty == null)
			{
				if (IsBindingRequestFor(bindingAttr, BindingFlags.NonPublic))
					representedProperty = _representedPrivateProperties.FirstOrDefault(p => p.Name.Equals(name, stringComparison));
				if (representedProperty == null)
					return null;
			}
		...
		private static IEnumerable<PropertyInfo> GetTranslatedPropertyRepresentations(Type type, Accessibility accessibility)
		{
			...
			var propertyTranslatedMethodGroups = type.GetMethods(bindingFlags)
