...
namespace VBScriptTranslator.CSharpWriter.CodeTranslation.Extensions
{
	public static class ScopeAccessInformation_Error_Extensions
	{
		...
		public static bool MayRequireErrorWrapping(this ScopeAccessInformation scopeAccessInformation, ICodeBlock codeBlock)
		{
			...
			// Locate where the specified code block exists within its scope - this returns a CodeBlockWithinScopeDefiningParent instance that has a
			// list of ancestors of the the codeBlock within the current scope-defining block. So, if the current code block is a value-setting statement
			// within a WHILE within a IF within a FUNCTION then the ParentsWithinScope list will the IF block and then the WHILE block (they are ordered
			// from the outside in).
			...
			// might identify false positives in some scenarios but should never identify false negatives. Note that the codeBlockWithinContent's list
			// of ancestors-within-the-scope-defining-parent does not include the scope-defining parent, so we need to insert that into the start of
			// the list (it's inserted into the started because the list is ordered from the most distant ancestor down to the direct parent).
			var currentCodeBlockInTrackBack = codeBlockWithinContent.CodeBlock;
			var remainingParentsToTrackBackThrough = codeBlockWithinContent.ParentsWithinScope.Insert(scopeAccessInformation.ScopeDefiningParent, 0);
			while (remainingParentsToTrackBackThrough.Any())
			{
				...
				{
					// If this is the block that we're trying to work out whether it needs to deal with errors or not then stop
					if (siblingCodeBlock == currentCodeBlockInTrackBack)
						break;
