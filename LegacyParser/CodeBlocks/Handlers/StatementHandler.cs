...
{
    /// <summary>
    /// This will handle tokens only if they contain no keywords - if they do then
    /// they should have been picked up by one of the other handlers (unless these
    /// keywords are properties of an object, in which case it is valid - eg.
    /// Response.End is valid despite "End" being a VBScript keyword). For
        ...
        public override ICodeBlock Process(List<IToken> tokens)
        {
            ...
            for (int index = 0; index < tokens.Count; index++)
            {
                ...
                if (tokens[index] is AtomToken)
                {
                    ...
                    {
                        // This is an AtomToken that does not appear to be a property
                        // of an object, so we need to ensure it's not a keyword that
                        // should have been handled already by this point
                        if (((AtomToken)tokens[index]).IsMustHandleKeyWord)
                            throw new Exception("Encountered must-handle keyword in statement content - this should have been handled by a previous AbstractBlockHandler: \"" + tokens[index].Content + "\"");
                    }
                }
