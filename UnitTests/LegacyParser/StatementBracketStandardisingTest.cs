	...
	public class StatementBracketStandardisingTest
	{
		public class NonChangingTests
		{
			...
			public void DirectMethodWithNoArgumentsAndNoBrackets()
			{
				...
				Assert.Equal(
					tokens,
					statement.GetBracketStandardisedTokens(withReferenceIfAny: null),
		...
		public class ChangingTests
		{
			...
			public void InitialWithReferencesWillBeExpandedForEaseOfManipulation()
			{
				...
				{
					new MemberAccessorOrDecimalPointToken(".", 0),
					new NameToken("Draw", 0),
					new StringToken("Test", 0)
				};
				var statement = new Statement(tokens, Statement.CallPrefixOptions.Absent);
				Assert.Equal(
					new IToken[]
					{
						new DoNotRenameNameToken("x", 0),
						new MemberAccessorOrDecimalPointToken(".", 0),
						new NameToken("Draw", 0),
						new OpenBrace(0),
						new StringToken("Test", 0),
					...
					statement.GetBracketStandardisedTokens(withReferenceIfAny: new DoNotRenameNameToken("x", 0)),
					new TokenSetComparer()
				);
			}
		}
