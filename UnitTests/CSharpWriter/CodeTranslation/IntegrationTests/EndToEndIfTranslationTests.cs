...
namespace VBScriptTranslator.UnitTests.CSharpWriter.CodeTranslation.IntegrationTests
{
	public class EndToEndIfTranslationTests
	{
		...
		/// <summary>
		/// There was a flaw in the translation process, where it is decided if ELSE IF conditionals can follow a simple if .. else if .. structure or if
		/// a more complicated if .. else { if .. } structure is required - if the first condition did not require any by-ref arguments to be mapped on to
		/// temporary variables but the second conditional DID require by-ref argument mapping (which is necessary if we're inside a function and we pass a
		...
		public class ByRefArgumentsOfContainingFunctionSpecialCases
		{
			...
			public void ByRefFunctionArgumentRequiresSpecialTreatmentIfUsedInConditionsAsByValArgumentWhenWithinErrorTrapping()
			{
				...
				var expected = new[]
				{
					...
					"    bool ifResult4;",
					"    object byrefalias3 = a;",
					"    ifResult4 = _.IF(() => _.CALL(this, _outer, \"F2\", _.ARGS.Val(_.CALL(this, byrefalias3, \"Name\"))), errOn2);",
					"    if (ifResult4)",
					"    {",
					"    }",
					"    _.RELEASEERRORTRAPPINGTOKEN(errOn2);",
